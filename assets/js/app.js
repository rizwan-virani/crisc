/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `CRISC` namespace with
...
(function () {
  ...
  var PASS = EX.passing;                    // passing scaled score (from CRISC.exam)
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("crisc." + k, fb); },
      set: function (k, v) { write("crisc." + k, v); },
      del: function (k) { try { localStorage.removeItem("crisc." + k); } catch (e) {} }
    };
  })();
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the CRISC is scored and delivered as a fixed-form, 150-question exam, and where the credential fits in an IT-risk, GRC, and governance career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question style of the CRISC &mdash; the high-stakes details every candidate should internalize before sitting the test.",
    ...
    g1.appendChild(card({
      ...
      desc: "Where CRISC sits on the career ladder: the IT-risk, control, and governance judgment it certifies, and the IT risk analyst, GRC analyst, and risk-and-control manager roles it unlocks.",
    ...
    b2.appendChild(el("p.sectsub", { text: "Four domains, each a rigorous reading interface with deep risk-and-control definitions, comparison tables, exam tips, and real-world scenarios." }));
    var g2 = el("div.grid");
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every CRISC acronym across all four domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    /* ---- 04 · Practice Quizzes ---- */
    var b4 = dashSection("05", "Practice quizzes", (S.questions ? S.questions.length : "400+") + " questions · 4 modes");
    b4.appendChild(el("p.sectsub", { html: "Drawn at random from a bank of <b>" + (S.questions ? S.questions.length : "400+") + "</b> original items, graded with full rationale. Domain quizzes isolate one objective set; the quick quiz mixes all four." }));
    ...
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the CRISC &mdash; " + EX.maxQuestions + " questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: the official ISACA CRISC exam content outline, canonical review manuals, video walkthroughs, and trusted community resources.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.CRISC_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. CRISC" });
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates CRISC.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.CRISC_VER ? "?v=" + window.CRISC_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "CRISC Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: CRISC", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where CRISC fits in your path", S.careerGuidance || [])); }
  ...
  /* ===========================================================================
     BOOT
     =========================================================================== */
  /* Populate the hero exam-line from CRISC.exam so the figures live in exactly
